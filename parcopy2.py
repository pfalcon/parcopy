# Based on Algorithm 21.6 from SSA Book (20180608), with fixes and
...
def sequentialize(copies, filter_dup_dests=False, allow_fan_out=True):
    ...
    while to_do:
        ...
        b = to_do.pop()
        log.debug("* to_do %s", b)
        # SSABook has an apparent typo, comparing for equality. The fix is
        # based both on testing and Algorithm 13 (page 81) in Boissinot thesis.
        if b != loc[pred[b]]:
            emit_copy(b, "tmp")
            loc[b] = "tmp"
            ready.append(b)
